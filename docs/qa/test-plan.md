...
**Soft requirements:** fog of war is the per-player explored-map visibility (automatic; the legacy GM fog toggle was removed), `has_line_of_sight` Bresenham correct (verified `test_session.py` / `test_visibility.py`); smooth movement — **broken** by BUG-003 (teleports instead of animating); session save/load to disk — **not implemented** (all state is in-memory `dict`s in `main.py`; no persistence). No zoom/pan (documented v1 trade-off).
...
- **WS message shapes match.** `welcome`/`state` carry `map, players, entities, you_entity, awareness`; `you:{id,name,role,entity_id}`; awareness items carry `entity_id,x,y,color,label`. Every field the frontend reads (`msg.map.name/width/height/cells`, `msg.entities`, `msg.you_entity`, `msg.awareness`, `msg.you.{id,name,role,entity_id}`) is present in `session.py` `state_for`/`welcome_for`. No field is read-but-unsent or sent-but-misnamed. Client→server sends (`join/request_state/move/place/create_entity/delete_entity/set_team/paint`) all match `handle_message`.
