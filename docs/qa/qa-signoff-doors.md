...
**Sign-off date:** 2026-09-03 (initial pass) · **Re-verification:** 2026-09-03,
commit `69e87a2` (both conditions cleared — see "Final verdict")
**QA verdict:** **PASS** (initially **PASS-WITH-CONDITIONS**; conditions cleared
in `69e87a2` — see "Final verdict")
...
- **AC3 exact-string deviation (BUG-DOORS-002):** in-process repro — a **player**
  ...
  occupancy) pins `"not allowed"`. **Re-verified after `69e87a2`:** the repro
  now returns `"not allowed"`, and the new regression test
  `test_player_lock_on_open_door_with_token_is_not_allowed` passes (suite 472).
...
| **AC3** state machine + permissions (exact) | **PASS** (caveat fixed in `69e87a2`) | `test_door_session.py::TestDoorStateMachine` (all 3×4 legal/illegal + exact strings: `not a doorway`, `destination out of bounds`, `action must be one of unlock/lock/open/close`, `door is locked`, `door is already unlocked/locked/open/closed`, `not allowed`), `test_validation_order_non_doorway_before_action`, `test_validation_order_state_before_role`, `test_player_lock_on_open_door_with_token_is_not_allowed` (added in `69e87a2`). **Original caveat:** one combination — a **player** `lock` on an **open** door **with a token** — returned the occupancy string instead of the §4.3-order `"not allowed"` (role-then-occupancy). See **BUG-DOORS-002 (P3)** — **fixed in `69e87a2`** (occupancy moved after the role gate per §4.3 #6→#7; all other error strings byte-identical, re-verified against `69e87a2`). All 11 error strings now exact. |
...
| **AC9** occupancy guard (D3, A5) | **PASS** | `test_door_session.py::TestDoorOccupancy` (`test_close_with_token_rejected`, `test_lock_while_open_with_token_rejected`, `test_lock_from_unlocked_not_guarded`, `test_player_lock_on_open_door_with_token_is_not_allowed` (added in `69e87a2`). Code: `_any_entity_at` guard on `close` and `lock`-from-`O`; `lock`-from-`U` unguarded. |
...
| **AC16** full regression | **PASS** | `pytest` **471 passed** + `unittest` **471 OK**; `e2e_proof.py` all-✓; `/health` ok; `app/grid.py` **byte-identical** (diff 0); `app/awareness.py` **byte-identical** (diff 0). **`69e87a2` re-run: `pytest` 472 passed (+1 = new BUG-DOORS-002 regression test) + `e2e_proof.py` 105/105 all-✓; `app/awareness.py` / `app/grid.py` still byte-identical to `b1ff47e` (re-verified). |

**AC audit summary:** 16 PASS (AC3's original PASS-WITH-CAVEAT —
**BUG-DOORS-002**, P3 — was **fixed in `69e87a2`**; the new regression test
pins the §4.3 order). No AC is failed. AC11 passes only after the
...
|---|---|---|---|
| **BUG-DOORS-001** | Cross-session door-state leak: all sessions on an unregistered id (incl. `"default"`) share the same mutable sample-dungeon `Grid`, so a door opened in one session is seen open in another | **P2** | **CONFIRMED** (live repro); **DOCUMENTED — accepted limitation** (README "Limitations" + link to this doc, `69e87a2`; not code-fixed, per this sign-off's recommendation — non-blocking, default single-browser UI unaffected) |
| **BUG-DOORS-002** | `_on_door` returns the occupancy error for a **player** `lock` on an **open** door that has a token, where the §4.3 pinned order (role before occupancy) returns `"not allowed"` | **P3** | **FIXED** (`69e87a2`: occupancy check moved after the role gate; regression test `test_player_lock_on_open_door_with_token_is_not_allowed`; re-run green — §9) |
...
all 16 acceptance criteria: 15 PASS and 1 PASS-WITH-CAVEAT (AC3) on the initial
pass. The
...
74/74 doors-live, all green on re-run). The two confirmed defects were both
...
was a single wrong error string on an illegal player action that is rejected
either way. The spec's incoherent §7.6/AC11(d) tap-mapping text was corrected
by errata to the coherent shipped behavior.

**Conditions for a clean PASS — both cleared in commit `69e87a2`:**

1. ~~fix-or-document **BUG-DOORS-001** (copy the sample grid per session, or
   document + limit the default-session flow)~~ → **DOCUMENTED (accepted
   limitation).** README "Limitations" now states the shared-live-grid behavior
   for sessions on unregistered map ids (door states and paints visible across
   sessions), notes that the normal upload/generate → `use_map` flow gives each
   map its own grid, and links
   [`docs/qa/BUG-DOORS-001.md`](BUG-DOORS-001.md) — consistent with the spec
   and with this sign-off's disposition (option (b)); deliberately **not**
   code-fixed (per the recommendation above). Verdict: **DOCUMENTED**.
2. ~~reorder the `lock`-from-open occupancy check to run after the role gate
   (or amend §4.3) and add the player-role regression test —
   **BUG-DOORS-002**~~ → **FIXED.** `app/session.py` `_on_door`: the
   `lock`-from-`O` occupancy guard now runs **after** the `unlock`/`lock` role
   gate, matching §4.3 order (#6 role → #7 occupancy; A5 extends the D3 guard
   to lock-from-open). The in-code docstring is updated to the 7-step order;
   the `close` occupancy guard and **all other error strings are
   byte-identical and in their original order** (verified against the
   `69e87a2` diff and a full read of `_on_door`). New regression test
   `tests/test_door_session.py::TestDoorOccupancy::test_player_lock_on_open_door_with_token_is_not_allowed`
   pins: player `lock` on open+token → `"not allowed"`; GM `close` and GM
   `lock` on the same door still → `"cannot close a door with a token on
   it"`; door stays `O`. Contract docs updated additively: PROJECT.md §4
   (additive `doors` field on `Grid`), §5/§6 (closed door blocks
   sight/movement like a wall; open is walkable/sight-transparent; GM
   override bypasses), §8 (REST map objects carry `doors`), §9 (WS `door`
   message + `map.doors` on `state`), all cross-referenced to
   `docs/design/door-features.md` (incl. the §7.6/AC11(d) and §7.1 errata).
   Verdict: **FIXED**.

**Final verification (re-run, `69e87a2`):** `pytest` **472 passed** (71
subtests; +1 = the new regression test) · `e2e_proof.py` **105/105 checks ALL
PASSED** · doors-live `qa_doors.py` **74/74 ALL PASSED** (from the initial
pass — not re-run this pass; the server-side paths it pins are covered by the
green e2e re-run). `app/awareness.py` / `app/grid.py` remain byte-identical to
baseline `b1ff47e`. No server left running; no code changes in this
re-verification (this doc only).

**Verdict: PASS** — both conditions cleared; the doors feature is approved to
ship with no open QA items.
